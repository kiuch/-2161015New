...
def analyze_sentiment(words):
    ...
    if positive_score > negative_score:
        ...
        sentiment = 'Negative'
    # ロジックの複雑化を避けるため、同数の場合はNeutralを維持
    else:
        sentiment = 'Neutral'
        
    return sentiment, positive_score, negative_score

def plot_sentiment_distribution(df_data, file_name, title): # df_dataに修正
    """感情極性の分布を円グラフで可視化する (色の対応を固定)"""
    
    # 🌟 修正箇所: 色の対応を固定 🌟
    fixed_order = ['Positive', 'Negative', 'Neutral'] # 順序を固定
    colors = ['#66b3ff', '#ff9999', '#99ff99'] # 青=Positive, 赤=Negative, 緑=Neutral
    
    sentiment_counts = df_data['Sentiment'].value_counts().reindex(fixed_order, fill_value=0) # 順序を適用し、欠損は0を埋める
